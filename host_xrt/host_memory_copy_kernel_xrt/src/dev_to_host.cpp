...
int main(int argc, char* argv[]) {
    ...
    parser.addSwitch("--data_size", "-s", "size of data", "0"); // Add this line
    ...
    int DATA_SIZE = 1024;
    DATA_SIZE = stoi(parser.value("data_size")); // Read the data_size value
   ...
   //auto cpy_run = copy_krnl(hostonly_bo0, deviceonly_bo0, DATA_SIZE, direction);
    ...
    double result = DATA_SIZE * sizeof(uint32_t);
    result /= (1000 * 1000); // to MB
    result /= kernel_time.count();   // to MB/s
    result *= run_count;   // to MB/s
        
    std::cout << "Throughput (Average of  " << run_count << ") :"  << result << " MB/s" << std::endl;
